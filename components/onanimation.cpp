...
OnAnimation::OnAnimation(QWidget *parent)
    : QWidget(parent)
    , m_outerRect(50, 50, 100, 100)
    , innerRect(175, 203, 50, 50) // Initialize innerRect
    , thickness(1.0)
    , m_opacity(1.0)
    , m_animationSpeed(2000) // Default animation speed
    , m_ringColor(QColor(0x08B0D5)) // Default ring color
{
...
void OnAnimation::setOuterRect(const QRectF &rect) {
    if (m_outerRect == rect)
        return;
    m_outerRect = rect;
...
void OnAnimation::setOpacity(qreal opacity) {
...
void OnAnimation::setAnimationSpeed(int speed) {
...
void OnAnimation::setRingColor(const QColor &color) {
...
void OnAnimation::paintEvent(QPaintEvent *event) {
...
void OnAnimation::startAnimation() {
    // Center coordinates
    ...
    QRectF startOuterRect(center.x() - 10, center.y() - 10, 20, 20); // Initial size and position of the ring
    QRectF endOuterRect(center.x() - 300, center.y() - 300, 600, 600); // Final size and position of the ring
